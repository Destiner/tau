...
const emit = defineEmits<{
  'file-preview': [preview: FilePreviewDescriptor, trigger: HTMLElement];
}>();
...
const rendered = computed(() => {
  ...
  if (!props.restrictImages) return html;
...
async function activateFilePath(
  ...
  if (!props.previewFiles || !props.basePath || filePreviewPending) return;
  ...
  try {
    ...
    const result = await invokeTraced<PreparedFilePreview>(
      'prepare_file_preview',
      {
        ...
        path: props.resolvePreviewPath?.(path) ?? path,
      },
    );
    ...
    emit(
      'file-preview',
      {
        ...
        directory: filePreviewDirectoryForPath(
          result.sourcePath,
          props.basePath,
        ),
        byteLength: result.byteLength,
        sourcePath: result.sourcePath,
        projectRoot: props.basePath,
        remoteIdentity: props.remoteProjectPath,
      },
      anchor,
    );
  } catch {
