...
| Extension slash commands                                           | Supported       | Supported       | Tau discovers them through `get_commands` and invokes them through `prompt`. Pi executes them instead of sending them to the model, so Tau does not show them as user messages. |
...
| Prompt reconciliation after a session replacement                  | Supported       | Supported       | Pi emits no replacement event, so Tau re-checks session identity when a command resolves, when a dialog is answered, and for a few seconds after a run settles.                 |
...
| Session names (`setSessionName`)                                   | Supported       | Supported       | Pi owns the name. Tau renames through `set_session_name`, adopts every `session_info_changed` Pi reports, and prefers the name in the session file over its own stored title.   |
