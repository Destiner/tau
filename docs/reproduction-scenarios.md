...
The `phantom-first-prompt-registration` scenario holds the pointer over a new session while its ordinary first prompt starts. Its gates expose the optimistic row before identity adoption, after Pi returns an empty preflight transcript, and after the assistant `message_end` barrier registers and selects the durable session. The user message and selected sidebar row remain continuously visible through every transition.
