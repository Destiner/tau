...
What a workflow writes is read as text rather than as a heading: a prompt's title and message, and the notifications beside them, are rendered as markdown and wrap. Links in them open in the default browser. File paths in local-session markdown open folders in the system file browser and files in their default application with Command-click on macOS and Control-click on Windows or Linux, resolved against the session's working directory. A remote project's files are on the other host, so its paths stay as text.
